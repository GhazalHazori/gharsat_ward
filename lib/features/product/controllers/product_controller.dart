...
import 'package:gharsat_ward/features/category/domain/models/find_what_you_need.dart';
import 'package:gharsat_ward/features/product/domain/models/home_category_product_model.dart';
import 'package:gharsat_ward/features/product/domain/models/most_demanded_product_model.dart';
...
class ProductController extends ChangeNotifier {
  ...
  void changeShippingMethod(int method) {
    ...
    // Reload products with new shipping method
    ...
    if (!_offsetList.contains(offset)) {
      ...
      ApiResponse apiResponse = await productServiceInterface!
          .getFilteredProductList(
              Get.context!, offset.toString(), _productType, title, shippingMethod: _shippingMethod);
  ...
  Future<void> getLProductList(String offset, {bool reload = false}) async {
    ...
    if (localData != null) {
      // Decode and parse product model
      final decoded = jsonDecode(localData.response);
      final productModel = ProductModel.fromJson(decoded);

      // Safely initialize product list
      _lProductList = [];
      if (productModel.products != null) {
        _lProductList!.addAll(productModel.products!);
      }

      // Safely assign page size
      _lPageSize = productModel.totalSize ?? 0;
    ...
    if (!_lOffsetList.contains(offset)) {
      _lOffsetList.add(offset);
      ApiResponse apiResponse =
          await productServiceInterface!.getLatestProductList(offset, shippingMethod: _shippingMethod);

      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        final responseData = apiResponse.response!.data;
        final productModel = ProductModel.fromJson(responseData);
        ...
        final cacheResponse = CacheResponseCompanion(
          endPoint: const Value(AppConstants.latestProductUri),
          header: Value(jsonEncode(apiResponse.response!.headers.map)),
        ...
        if (localData != null) {
          ...
          await database.insertCacheResponse(cacheResponse);
        }
      } else {
        ApiChecker.checkApi(apiResponse);
      }
  ...
  Future<void> initBrandOrCategoryProductList(
    ...
    ApiResponse apiResponse = await productServiceInterface!
        .getBrandOrCategoryProductList(
            isBrand: isBrand, id: id!, offset: offset, shippingMethod: _shippingMethod);
  ...
  void initRelatedProductList(String id, BuildContext context) async {
    ApiResponse apiResponse =
        await productServiceInterface!.getRelatedProductList(id, shippingMethod: _shippingMethod);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _relatedProductList = [];
  ...
  Future<void> getFeaturedProductList(String offset,
    ...
    if (!_featuredOffsetList.contains(offset)) {
      ...
      ApiResponse apiResponse =
          await productServiceInterface!.getFeaturedProductList(offset, shippingMethod: _shippingMethod);
      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        if (offset == '1') {
          _featuredProductList = [];
  ...
  Future<void> getRecommendedProduct() async {
    ...
    ApiResponse apiResponse =
        await productServiceInterface!.getRecommendedProduct(shippingMethod: _shippingMethod);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<void> getHomeCategoryProductList(bool reload) async {
    if (_homeCategoryProductList.isEmpty || reload) {
      ...
      ApiResponse apiResponse =
          await productServiceInterface!.getHomeCategoryProductList(shippingMethod: _shippingMethod);
      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
  ...
  Future<void> getMostDemandedProduct() async {
    ...
    ApiResponse apiResponse =
        await productServiceInterface!.getMostDemandedProduct(shippingMethod: _shippingMethod);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<void> findWhatYouNeed() async {
    ...
    ApiResponse apiResponse =
        await productServiceInterface!.getFindWhatYouNeed(shippingMethod: _shippingMethod);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      findWhatYouNeedModel =
  ...
  Future<void> getJustForYouProduct() async {
    ...
    justForYouProduct = [];
    ApiResponse apiResponse =
        await productServiceInterface!.getJustForYouProductList(shippingMethod: _shippingMethod);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      apiResponse.response?.data.forEach(
          (justForYou) => justForYouProduct?.add(Product.fromJson(justForYou)));
  ...
  Future<void> getMostSearchingProduct(int offset,
    ...
    ApiResponse apiResponse =
        await productServiceInterface!.getMostSearchingProductList(offset, shippingMethod: _shippingMethod);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (apiResponse.response?.data['products'] != null &&
  ...
  Future<void> getDiscountedProductList(int offset, bool reload,
    ...
    ApiResponse apiResponse = await productServiceInterface!
        .getFilteredProductList(Get.context!, offset.toString(),
            ProductType.discountedProduct, title, shippingMethod: _shippingMethod);
  ...
  Future<void> getClearanceAllProductList(String offset,
    ...
    ApiResponse apiResponse =
        await productServiceInterface!.getClearanceAllProductList(offset, shippingMethod: _shippingMethod);
